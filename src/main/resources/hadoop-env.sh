      ...
      SHARED_HDFS_NAMENODE_OPTS="-server -XX:ParallelGCThreads=8 -XX:+UseConcMarkSweepGC -XX:ErrorFile=/var/log/hadoop/$USER/hs_err_pid%p.log -XX:NewSize=384m -XX:MaxNewSize=384m -Xloggc:/var/log/hadoop/$USER/gc.log-`date +'%Y%m%d%H%M'` -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps -XX:CMSInitiatingOccupancyFraction=70 -XX:+UseCMSInitiatingOccupancyOnly -Xms2048m -Xmx2048m -Dhadoop.security.logger=INFO,DRFAS -Dhdfs.audit.logger=INFO,DRFAAUDIT"
      ...
      export HDFS_NAMENODE_OPTS="$HDFS_NAMENODE_OPTS -Djava.security.auth.login.config=/usr/hdp/3.0.0.0-1634/hadoop/conf/hdfs_nn_jaas.conf -Djavax.security.auth.useSubjectCredsOnly=false"
      export HDFS_SECONDARYNAMENODE_OPTS="$HDFS_SECONDARYNAMENODE_OPTS -Djava.security.auth.login.config=/usr/hdp/3.0.0.0-1634/hadoop/conf/hdfs_nn_jaas.conf -Djavax.security.auth.useSubjectCredsOnly=false"
      export HDFS_DATANODE_OPTS="$HDFS_DATANODE_OPTS -Djava.security.auth.login.config=/usr/hdp/3.0.0.0-1634/hadoop/conf/hdfs_dn_jaas.conf -Djavax.security.auth.useSubjectCredsOnly=false"
      export HADOOP_JOURNALNODE_OPTS="$HADOOP_JOURNALNODE_OPTS -Djava.security.auth.login.config=/usr/hdp/3.0.0.0-1634/hadoop/conf/hdfs_jn_jaas.conf -Djavax.security.auth.useSubjectCredsOnly=false"
      ...
      export JAVA_LIBRARY_PATH=${JAVA_LIBRARY_PATH}:/usr/hdp/3.0.0.0-1634/hadoop/lib/native/Linux--64
